...
if df.empty:
    st.error("Dataset kosong atau gagal load")
    st.stop()
...
df_2018 = df[df['order_year'] == 2018]  # pakai full data, bukan filtered

if df_2018.empty:
    st.warning("Tidak ada data untuk tahun 2018")
else:
    ...
    if state.empty:
        ...
        st.write(f"""
        **Insight:**
        - Provinsi dengan revenue tertinggi adalah **{top_state}**
        - Menjadi pasar utama yang perlu dipertahankan
        """)
        ...
        if df_top.empty:
            st.warning("Data RFM tidak tersedia")
        else:
            ...
            if len(rfm) < 4:
                st.warning("Data terlalu sedikit untuk RFM")
            else:
